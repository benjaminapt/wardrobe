...
for (const outfit of outfitsData.outfits) {
  if (outfit.status === 'planned') {
    ...
    for (const garmentId of outfit.garmentIds) {
      ...
      if (garment) {
        // Find the cutout image path. It's usually data/imported/<id>-garment.png
        const assetName = garment.assetName;
        const localPath = `data/imported/${assetName}`;
    ...
    prompt += `2. Use the 'generate_image' tool with the reference paths provided above. The ImageName should be '${outfit.id}'.\n`;
